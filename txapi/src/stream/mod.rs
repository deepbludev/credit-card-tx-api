...
pub async fn init_transactions_channel() -> (
    ...
    let (msg_tx, msg_rx) = broadcast::channel(buffer_size);

    // combine all streams into a single consolidated stream
    let mut msg_stream = select_all(vec![
        stream_tx_from_mocks(),
    ...
    tokio::spawn(async move {
        while let Some(transaction) = msg_stream.next().await {
            // ignore send errors (occurs when no receivers)
            // TODO: handle this gracefully
            let _ = msg_tx_clone.send(transaction);
        }
    });
    (msg_tx, msg_rx)
}
